...
CREATE TABLE IF NOT EXISTS `bbdd_ventas`.`PEDIDOS` (
  ...
  INDEX `ind_pedido_cliente` (`codigo_customer` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bbdd_ventas`.`VENDEDORES_PEDIDOS` (
  `codigo_seller` VARCHAR(10) NOT NULL,
  `codigo_pedido` VARCHAR(10) NOT NULL,
  INDEX `ind_credencial_pedido_vendedor` (`codigo_seller` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `bbdd_ventas`.`LISTA_PRODUCTOS_COMPRADOS` (
  ...
  INDEX `ind_lista_prod_comp_productos` (`codigo_producto` ASC) INVISIBLE,
  INDEX `ind_lista_prod_comp_compra` (`codigo_compra` ASC) VISIBLE,
